...
// The house's Music Assistant address and the guest login are NOT kept in source control. They come
// from the gitignored local.properties (see local.properties.example) and are turned into a
// generated BrandSecrets object at build time, which BrandConfig reads.
val brandLocalProps = Properties().apply {
    val f = rootProject.file("local.properties")
    if (f.exists()) f.inputStream().use { load(it) }
}
val brandSecretsDir = layout.buildDirectory.dir("generated/brandSecrets/kotlin")
val generateBrandSecrets = tasks.register("generateBrandSecrets") {
    description = "Generates BrandSecrets (server address + guest login) from local.properties."
    group = "build setup"
    val host = brandLocalProps.getProperty("chasesretreat.server.host", "")
    val port = brandLocalProps.getProperty("chasesretreat.server.port", "8095").toInt()
    val tls = brandLocalProps.getProperty("chasesretreat.server.tls", "false").toBoolean()
    val username = brandLocalProps.getProperty("chasesretreat.server.username", "")
    val password = brandLocalProps.getProperty("chasesretreat.server.password", "")
    inputs.property("host", host)
    inputs.property("port", port)
    inputs.property("tls", tls)
    inputs.property("username", username)
    inputs.property("password", password)
    outputs.dir(brandSecretsDir)
    doLast {
        fun esc(v: String) = v.replace("\\", "\\\\").replace("\"", "\\\"").replace("$", "\\$")
        if (host.isEmpty()) {
            logger.warn(
                "chasesretreat.server.* is not set in local.properties: the Music tab will show " +
                    "\"connect to Wi-Fi\" because there is no server to reach. See local.properties.example.",
            )
        }
        val out = brandSecretsDir.get().asFile.resolve("io/music_assistant/client/branding/BrandSecrets.kt")
        out.parentFile.mkdirs()
        out.writeText(
            """
            package io.music_assistant.client.branding

            // GENERATED by generateBrandSecrets from local.properties. Do not edit or commit.
            internal object BrandSecrets {
                const val SERVER_HOST = "${esc(host)}"
                const val SERVER_PORT = $port
                const val SERVER_IS_TLS = $tls
                const val SERVER_USERNAME = "${esc(username)}"
                const val SERVER_PASSWORD = "${esc(password)}"
            }
            """.trimIndent() + "\n",
        )
    }
...
kotlin {
    ...
    sourceSets {
        commonMain {
            kotlin.srcDir(generateBrandSecrets.map { brandSecretsDir.get().asFile })
        }
