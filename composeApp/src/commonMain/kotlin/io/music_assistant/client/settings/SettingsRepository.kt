...
class SettingsRepository(
    ...
    // This build is locked to a single property's server: the connection is a compile-time
    // constant (BrandConfig), never read from or written to user-editable storage, so there is
    // no "add/change server" path for a guest to find.
    private val _connectionInfo = MutableStateFlow(BrandConfig.lockedConnectionInfo)
    val connectionInfo = _connectionInfo.asStateFlow()

    @Suppress("UnusedParameter") // Kept so call sites elsewhere don't need to change.
    fun updateConnectionInfo(connectionInfo: ConnectionInfo?) {
        // Locked build: the server address never changes.
    }
